...
const SimpleKitchenSinkVideo = lazy(() => import('./SimpleKitchenSinkVideo'));

interface SimpleKitchenSinkVideoLazyProps {
  gameId: string;
...
/**
 * Lazy-loaded wrapper for SimpleKitchenSinkVideo that code-splits the Daily SDK.
 * This ensures the Daily SDK (240kB+ minified) is only loaded when video functionality is needed.
 */
export default function SimpleKitchenSinkVideoLazy(props: SimpleKitchenSinkVideoLazyProps) {
  return (
    <Suspense fallback={<VideoLoadingFallback />}>
      <SimpleKitchenSinkVideo {...props} />
    </Suspense>
  );
}