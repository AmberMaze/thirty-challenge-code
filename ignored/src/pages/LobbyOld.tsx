...
export default function Lobby() {
  ...
  debugLog('Lobby', 'component_render', {
    gameId,
          ...
          if (!result.success) {
            ...
            if (role === 'host') {
              try {
                ...
                currentShowAlertMessage(
                  currentT('sessionCreatedSuccessfully'),
                  'success',
                );
              } catch (createError) {
                console.error('Failed to create new game:', createError);
                currentShowAlertMessage(
                  currentT('failedCreateSession'),
                  'error',
                );
              }
            } else {
              ...
              currentShowAlertMessage(
                `${currentT('failedLoadSessionData')}: ${result.error}`,
                'error',
              );
            }
          }
